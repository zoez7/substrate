...
// This file keeps the CRD-to-proto template projection alive for test
// fixtures only: many unit tests still declare their template fixtures as
// CRDs. Production resolution is store-only (template_convert.go). Delete
// this once those fixtures build ateapipb.ActorTemplate protos directly.

// mustTemplateFromCRD converts a fixture that must be convertible.
